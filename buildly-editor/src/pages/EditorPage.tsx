        ...
        >
          ...
          {site.pages.length > 1 && (
            <span
              onClick={e => {
                e.stopPropagation();
                if (confirm(`Delete "${page.name}"?`)) removePage(page.id);
              }}
      ...
      {adding ? (
        <div className="flex items-center gap-1">
          <input
            ...
            className="bg-surface border border-accent/50 rounded px-2 py-1 text-xs text-text outline-none w-28"
            placeholder="Page name"
          />
          <button onClick={handleAdd} className="text-xs text-accent px-2 py-1 hover:bg-white/5 rounded">✓</button>
          <button onClick={() => setAdding(false)} className="text-xs text-muted px-1 py-1 hover:bg-white/5 rounded">✕</button>
        </div>
      ) : (
        <button
...
export const EditorPage: React.FC = () => {
  ...
  // ── Fetch site ──────────────────────────────────────────────────────────────
  // NOTE: TanStack Query v5 removed onSuccess — use useEffect watching data instead.
  const { data: fetchedSite, isLoading, error } = useQuery({
    queryKey: ['site', id],
    queryFn: () => sitesApi.get(id!).then(r => r.data.data as Site),
    enabled: !!id,
    staleTime: Infinity,   // canvas is source of truth after first load
    retry: 1,
  });

  // Load site into store the moment query resolves
  useEffect(() => {
  ...
  const doSave = useCallback(async () => {
    ...
    setIsSaving(true);
    try {
      await sitesApi.update(site._id, {
        pages: site.pages,
        meta: site.meta,
        name: site.name,
      });
      // Don't call setSite(res.data.data) — that would overwrite local state
      // with server data that may have stripped unknown fields via DTO whitelist.
      // Just mark clean; the store already has the correct state.
      setIsDirty(false);
    } catch {
      ...
      <header className="h-12 bg-surface border-b border-border flex items-center px-3 gap-2 flex-shrink-0 z-50">
        ...
        <div className="text-xs ml-2 flex items-center gap-1.5">
          {isSaving
            ...
            : isDirty
            ? <span className="text-yellow-400/70">● Unsaved</span>
        ...
        <div className="flex items-center gap-0.5">
          <Tooltip label="Desktop preview">
            <Button
              variant={previewMode && !mobilePreview ? 'accent' : 'ghost'}
              size="sm"
          ...
          {previewMode && (
            <Tooltip label="Exit preview">
              <Button variant="outline" size="sm" icon={<EyeOff size={14} />} onClick={() => setPreviewMode(false)}>
                Exit
              </Button>
            </Tooltip>
          )}
        ...
        {site.status === 'published' ? (
          <>
            ...
            <Button variant="outline" size="sm" icon={<WifiOff size={14} />} onClick={handleUnpublish}>
              Unpublish
            </Button>
          </>
        ) : (
