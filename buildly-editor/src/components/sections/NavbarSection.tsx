...
export const NavbarSection: React.FC<Props> = ({ section, pages = [] }) => {
  ...
  const fontStyle: React.CSSProperties = {
    fontFamily: styles.headingFont ? `"${styles.headingFont}", sans-serif` : 'Syne, sans-serif',
  };
  const bodyFont: React.CSSProperties = {
    fontFamily: styles.bodyFont ? `"${styles.bodyFont}", sans-serif` : 'DM Sans, sans-serif',
  };
  ...
  if (variant === 1) return (
    <nav style={{ ...navBase, borderBottom: `1px solid ${styles.borderColor}` }} className="w-full sticky top-0 z-50">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
        <a href="/" style={{ ...fontStyle, color: styles.headingColor }} className="font-bold text-lg sm:text-xl shrink-0">{c.brand}</a>
        ...
        <div style={{ background: styles.bg, borderTop: `1px solid ${styles.borderColor}` }} className="md:hidden px-4 py-4 flex flex-col gap-3">
          ...
          {c.ctaText && (
            <NavLink href={c.ctaLink || '#'} label={c.ctaText}
              ...
              style={{ background: styles.headingColor, color: styles.bg }} />
          )}
        </div>
      )}
    </nav>
  ...
  if (variant === 2) return (
    <nav style={navBase} className="w-full sticky top-0 z-50">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
        <a href="/" style={{ ...fontStyle, color: styles.headingColor }} className="font-bold text-xl tracking-tight shrink-0">{c.brand}</a>
    ...
    <nav style={{ ...navBase, borderBottom: `1px solid ${styles.borderColor}` }} className="w-full sticky top-0 z-50">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-center h-10 pt-3">
          <a href="/" style={{ ...fontStyle, color: styles.headingColor }} className="font-bold text-2xl">{c.brand}</a>
        </div>
        <div className="hidden md:flex items-center justify-center gap-6 lg:gap-8 h-10 pb-2">
          ...
          {c.ctaText && (
            <NavLink href={c.ctaLink || '#'} label={c.ctaText}
              ...
              style={{ background: styles.accentColor, color: styles.bg }} />
          )}
        </div>
